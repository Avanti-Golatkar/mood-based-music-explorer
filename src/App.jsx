...
(function captureAuthParams() {
  ...
  if (code) {
    ...
    // Strip the ?code= from the URL immediately so it won't be reused
  ...
  const search = useCallback(async (moodId) => {
    ...
    try {
      const sRes = await fetch(
        `https://api.spotify.com/v1/search?q=${encodeURIComponent(mood.spotifyQuery)}&type=playlist&limit=8&market=US`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (sRes.status === 401) { dispatch({ type:SA.ERROR, err:"Session expired — please reconnect Spotify." }); return; }
      const sData    = await sRes.json();
      const items    = sData.playlists?.items?.filter(Boolean) ?? [];
      if (!items.length) { dispatch({ type:SA.ERROR, err:"No playlists found for this mood on Spotify." }); return; }
  ...
  return (
    <nav style={S.nav}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        {/* Active mood pill — shows current mood emoji + label in its accent colour */}
        {activeMood && (
          <div style={{
            display:"flex",alignItems:"center",gap:6,padding:"5px 13px",borderRadius:20,
            background:`${activeMood.palette.accent}18`,
            border:`1px solid ${activeMood.palette.accent}55`,
            transition:"all 0.5s",
          }}>
            <span style={{fontSize:13}}>{activeMood.emoji}</span>
            <span style={{fontSize:11,fontWeight:600,color:activeMood.palette.accent,letterSpacing:0.5}}>
              {activeMood.label}
            </span>
          </div>
        )}
